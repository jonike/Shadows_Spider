...
float Anim::cubicHerm_(float t, float p0, float p1, float m0, float m1)
{
    auto t2 = t * t;
    auto t3 = t2 * t;
    auto herm = (2.f * t3 - 3.f * t2 + 1.f) * p0 + (t3 - 2.f * t2 + t) * m0 + (-2.f * t3 + 3.f * t2) * p1 + (t3 - t2) * m1;
...
float Anim::dynAnim(string type)
{
    ...
    if (type == "smoothStep")
    {
        auto smoothStep = lin * lin * (3.f - 2.f * lin);
    ...
    {
        auto springF = .2f;
        auto spring = pow(2.f, -10.f * lin) * sin((lin - springF / 4.f) * (2.f * PI) / springF) + 1.f;
    ...
    if (type == "cycle")
    {
        auto cycle = sin(2.f * cycles * PI * time);
//        auto cycle = sin(2.f * cycles * PI * time) * 4;
    ...
    if (type == "accelDecel")
    {
        auto accelDecel = cos((lin + 1.f) * PI) / 2.f + .5f;
    ...
    {
        auto anticipF = 4.f;
        auto anticp = lin * lin * ((anticipF + 1.f) * lin - anticipF);
    ...
    {
        auto anticipOverShootF = 4.f * 1.5f;
    ...
    if (type == "cubicHerm")
    {
        auto cubicHerm = cubicHerm_(lin, 0.f, 1.f, 4.f, 4.f);// * cycle;
