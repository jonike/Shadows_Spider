...
layout(bindless_sampler, location = 10) uniform sampler2DShadow shadowM1;
layout(bindless_sampler, location = 11) uniform sampler2DShadow shadowM2;
...
vec4 allShadowCoords[3] = { v.shadowCoord0, v.shadowCoord1, v.shadowCoord2 };
sampler2DShadow allSampler2DShadow[3] = { shadowM0, shadowM1, shadowM2 };
...
//        ruffT = pow(ruffT, 2.2f); ////gamma space input tex req decoding
    ...
    float shadowRead0, shadowRead1, shadowRead2;
...
//    gBuf2.w = packHalf2x16(v.UV); //
    ...
    gBuf4.w = packHalf2x16(vec2(shadowRead2, 0.f));
